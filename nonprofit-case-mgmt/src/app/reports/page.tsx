...
// place holder for diaplay purposes
...
const initialEntries: ServiceEntry[] = [
  {
    ...
    date: "2026-03-10",
    serviceType: "s1",
    staffMember: "Victoria Justice",
    notes: "Met with client to discuss case plan and next steps.",
  },
  {
    id: "r2",
    clientId: "1",
    date: "2026-03-18",
    serviceType: "s4",
    staffMember: "Polly Pocket",
    notes: "Provided transportation support for scheduled appointment.",
  },
  {
    id: "r3",
    clientId: "2",
    ...
    serviceType: "s5",
    staffMember: "Dr Phil",
    notes: "Provided food pantry resources and follow-up instructions.",
  },
];
...
export default function ReportsPage() {
  ...
  // BACKEND WIRING: maybe? i think so? as of now runs local
  ...
  // form data holders
  // BACKEND WIRING: submit logic will post to backend
  const [formData, setFormData] = useState({
    date: "",
    serviceType: initialServiceTypes[0] ?? "",
    notes: "",
  });
  ...
  const handleAddEntry = () => {
    ...
    // new service log instance, wires to the form
    // BACKEND WIRING: this is one of the main spots to replace local state update 
    const newEntry: ServiceEntry = {
      id: crypto.randomUUID(), // BACKEND WIRING. just threw something in for rendering purposes
      clientId: selectedClientId,
      date: formData.date,
      serviceType: formData.serviceType,
      staffMember: getCurrentStaffMember(), // BACKEND WIRING: should come from authenticated user/session
      notes: formData.notes.trim(),
    };

    // temp local update for UI display
    // BACKEND WIRING
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <Button onClick={handleAddEntry} className="rounded-xl">
                <Plus className="mr-2 h-4 w-4" />
                Save Log
              </Button>
            </div>
